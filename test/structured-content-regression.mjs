...
console.log(`${BLUE}🧪 FRED Structured Content Regression Tests${RESET}`);

const indexContent = readFile('src/index.ts');
assertContains('src/index.ts', indexContent, 'FredDataDO', 'index.ts exports FredDataDO');
...
assertContains('src/tools/code-mode.ts', codeModeContent, 'fredCatalog', 'code-mode.ts imports fredCatalog');
...
assertContains('src/spec/catalog.ts', catalogContent, 'FRED', 'catalog.ts names the API');
...
console.log(`\n${GREEN}✅ FRED structured content regression tests passed.${RESET}`);
