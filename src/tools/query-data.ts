...
interface QueryEnv {
    FRED_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv) {
    const handler = createQueryDataHandler("FRED_DATA_DO", "fred");

    server.registerTool(
        "fred_query_data",
        {
            title: "Query Staged FRED Data",
            description:
                "Query staged FRED economic data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
